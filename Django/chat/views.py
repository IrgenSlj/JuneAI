...
class OllamaManager:
    def __init__(self):
        self.port = getattr(settings, 'OLLAMA_PORT', 11435) # this was 11434
...
class JuneAI:
    def __init__(self):
        self.url = 'http://localhost:11435/api/chat'  # this was 11434
        self.session = None
        self._ensure_ollama_available()
        self.session = self._create_session()

    def _ensure_ollama_available(self):
        if not OllamaConnection.is_port_in_use(11435):  # this was 11434
            logger.warning("Ollama not detected. Please start Ollama service.")
            if not OllamaConnection.wait_for_port(11435):  # this was 11434
