...
schema), so recurring-task columns need their own migration path.
"""
...
def ensure_tasks_migration(conn: sqlite3.Connection) -> None:
    """Add recurring-task columns if they don't exist (idempotent)."""
    existing = {
        row["name"]
        for row in conn.execute("PRAGMA table_info(tasks)").fetchall()
    }
    for col, col_type in _RECURRING_COLS.items():
